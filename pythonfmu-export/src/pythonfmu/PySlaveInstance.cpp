...
void PySlaveInstance::SetRealArrayEx(const cppfmu::FMIValueReference* vr, std::size_t nvr, cppfmu::FMIReal const* values, std::size_t size)
{
    ...
    //             PyObject* value = Py_BuildValue("d", values[j]);
    //             if (!value) {
    //                 std::cerr << "Failed to create PyObject at index " << j << std::endl;
    //                 continue;
    //             }
    //             PyList_SetItem(refs, j, value);
    //         }
