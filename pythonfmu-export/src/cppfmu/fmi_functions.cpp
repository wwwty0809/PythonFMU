    ...
    size_t size)
{
    ...
    std::cout << "Calling fmi2GetRealArrayEx function" << std::endl;
    std::cout << "Number of value references: " << nvr << std::endl;
    
    try {
        component->slave->GetRealArray(vr, nvr, value, size);
        
        // Print success message
        std::cout << "fmi2GetRealArrayEx successful" << std::endl;
    ...
    size_t size)
{
    const auto component = reinterpret_cast<Component*>(c);
    try {
        component->slave->SetRealArrayEx(vr, nvr, value, size);
        return fmi2OK;
    } catch (const cppfmu::FatalError& e) {
        component->logger.Log(fmi2Fatal, "", e.what());
        return fmi2Fatal;
