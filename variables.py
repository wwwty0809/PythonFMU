...
class RealArray(ScalarVariable):
    def __init__(self, name: str, start: Optional[List[float]] = None, **kwargs):
        """
        Initializes a RealArray variable.

        :param name: Name of the variable
        :param start: Optional starting values for the array (list of floats)
        :param kwargs: Additional arguments for the parent class
        """
        super().__init__(name, **kwargs)
        self.__attrs = {"start": start or []}

    @property
    def start(self) -> List[float]:
        """Gets the starting values for the array."""
        return self.__attrs["start"]

    @start.setter
    def start(self, value: List[float]):
        """Sets the starting values for the array."""
        if not isinstance(value, list) or not all(isinstance(v, (int, float)) for v in value):
            raise ValueError("Start values must be a list of floats or integers.")
        self.__attrs["start"] = value

    def to_xml(self) -> Element:
        """
        Converts the RealArray to an XML element.

        :return: XML element representing the RealArray.
        """
        attrib = {}
