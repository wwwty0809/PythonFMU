...
from .variables import Boolean, Integer, Real, ScalarVariable, String, RealArray
...
class Fmi2Slave(ABC):
    ...
    def get_real_array(self, vrs: List[int]) -> List[List[float]]:
        """
        Retrieve array values based on value references.

        :param vrs: List of value references
        :return: List of arrays corresponding to the value references
        """
        refs = []
        for vr in vrs:
            var = self.vars[vr]
    ...
    def set_real_array(self, vrs: List[int], values: List[List[float]]):
        """
        Set array values based on value references.

        :param vrs: List of value references
        :param values: List of arrays to assign to the variables
        """
        for vr, value in zip(vrs, values):
            var = self.vars[vr]
            if isinstance(var, RealArray):
